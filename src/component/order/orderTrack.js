...
export default function OrderTrackerCard() {
    ...
    const handleAdvanceStatus = async () => {
        ...
        const patchEndpointMap = {
            confirmed: `/order/${order.id}/payment/mock`,
            preparing: `/order/${order.id}/ready`,
            ready: `/order/${order.id}/pickedup`,
        };

        const current = order.status;
        const nextIndex = statusFlow.indexOf(current) + 1;
        const nextStatus = statusFlow[nextIndex];

        if (!patchEndpointMap[current] || nextIndex >= statusFlow.length) {
            router.replace("/(main)/history");
            return;
        }

        try {
            await api.patch(patchEndpointMap[current], null, {
                headers: { Authorization: `Bearer ${token}` },
            });
            const updatedOrder = { ...order, status: nextStatus };
            setOrders([updatedOrder, ...orders.slice(1)]);
        } catch (error) {
            console.error(`❌ Failed to update from ${current}:`, error.response?.data || error.message);
        }
    };
